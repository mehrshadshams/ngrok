...
async fn handle_control_connection(
    ...
    tokio::spawn(async move {
        ...
        // Cleanup state
        let mut guard = state.lock().await;
        // Check if the current control is still US, if so, remove it.
        // We can't easily check equality of Control, but if we assume one client at a time:
        if guard.client_control.is_some() {
            info!("Clearing active client session");
            guard.client_control = None;
        }
    });
}
